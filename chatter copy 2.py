...
def apply_settings_json(settings_json):
    ...
    try:
        ...
        return [
            ...
            # 3 ref_audio_input (filepath string)
            ...
            # 10 whisper_model_dropdown (label)
            wm,
        ...
        with gr.Tabs():
            ...
            def collect_ui_settings(*vals):
                ...
                if len(keys) != len(vals):
                    raise ValueError(
                        f"[SETTINGS ERROR] collect_ui_settings: Number of values ({len(vals)}) does not match keys ({len(keys)})!")
            ...
            run_button.click(
                fn=lambda *args: (
                    # text_input + rest of option fields (skipping file/audio)
                    collect_ui_settings(*([args[0]] + list(args[3:]))),
